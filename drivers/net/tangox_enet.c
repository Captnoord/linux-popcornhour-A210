...
static void enet_link_check(unsigned long data)
{
	...
	if(ret && !link_up)
	{
		val = enet_mdio_read(dev, priv->mii.phy_id, 0x1B);	//Read from register 27
		val |= (1<<11);										//Set bit 11 to "1", ON.
		enet_mdio_write(dev, priv->mii.phy_id, 0x1B, val);	//Write the value to register.
		link_up = 1;
	}
	else if(!ret && link_up)
	{
		/*Off the LNK LED when link down*/
		val = enet_mdio_read(dev, priv->mii.phy_id, 0x1B);	//Read from register 27	
		val &= ~(1<<11);									//Clear bit 11
		enet_mdio_write(dev, priv->mii.phy_id, 0x1B, val);	//Write the value to register.
		link_up = 0;
